...
### 機能要件1: 予約日時設定（F-04-2）
- 機能仕様1: 予約日時の指定（日時ピッカー）
- 機能仕様2: 過去の日時は選択不可（バリデーション）

### 機能要件2: 自動公開バッチ処理（F-04-2）
- 機能仕様1: 指定日時に自動公開（バッチ処理）
...
- 機能仕様1: 予約記事の一覧表示
- 機能仕様2: 予約日時の変更
- 機能仕様3: 予約のキャンセル（下書きに戻す）

### 機能要件4: 通知（F-04-2）
- 機能仕様1: 通知機能の有無・実装方式: TBD
  - 予約日時のタイムゾーン扱い: TBD
...
### 非機能要件1: パフォーマンス
- 非機能仕様1: バッチ処理: 毎分実行、1分以内に完了
- 非機能仕様2: 予約記事一覧: 3秒以内に表示

### 非機能要件2: 信頼性
- 非機能仕様1: バッチ処理の失敗時はログ記録
- 非機能仕様2: バッチ処理の二重実行防止

### 非機能要件3: UX
- 非機能仕様1: 予約日時は分単位で指定可能
- 非機能仕様2: 予約キャンセル後は下書きに戻る
...
| F-04-2 | 予約投稿機能自身 |
| F-04-3 | 予約公開された記事が記事一覧に表示される |
| F-04-4 | 予約公開された記事が人気記事算出の対象 |
| F-04-5 | 予約公開された記事が詳細ページで表示される |
...
| **テスト** | | |
| 単体テスト | 2sp | バッチ公開処理・キャンセル・再スケジュールの単体テスト |
| E2Eテスト | 2sp | 予約作成→一覧反映→予約変更/キャンセルの主要フロー |
