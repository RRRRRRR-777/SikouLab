...
### 機能要件2: 目次機能（F-04-5）
- 機能仕様3: H2、H3見出しから目次を自動生成
- 機能仕様4: 目次クリックで該当見出しへスクロール
- 機能仕様5: 現在位置の見出しをハイライト
  - 目次の表示位置: TBD

### 機能要件3: 関連銘柄表示（F-04-5）
- 機能仕様6: 関連銘柄を表示
  - 関連銘柄のリアルタイム価格表示: TBD
- 機能仕様7: 銘柄クリックで銘柄詳細ページへ遷移

### 機能要件4: いいね・ブックマーク操作（F-04-5）
- 機能仕様8: いいねボタンでトグル操作
- 機能仕様9: ブックマークボタンでトグル操作
...
- 非機能仕様4: エラー発生時は適切なエラーメッセージ表示
- 非機能仕様5: 記事が存在しない場合は404ページへ遷移
