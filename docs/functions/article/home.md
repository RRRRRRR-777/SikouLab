...
- 機能仕様2: メインコンテンツに人気記事を4件表示する
  - 最新1件を大きく、残り3件を小さく表示

### 機能要件3: 右ペイン人気記事表示（F-04-4）
- 機能仕様3: 右ペインに人気記事を5件表示する
  - コンパクトなリスト形式
  - 記事タイトルのみ

### 機能要件4: ジャンル別記事切り替え（F-04-3）
- 機能仕様4: タブバーでジャンル別記事を切り替える
  - Homeタブ: 全記事を表示
  - ジャンルタブ: 該当ジャンルの記事のみ表示

### 機能要件5: 記事詳細遷移（F-04-5）
- 機能仕様5: 記事カードから詳細ページへ遷移する

### 機能要件6: サイドバー展開状態永続化
- 機能仕様6: サイドバー記事タブの展開状態を保存し永続化する

### 機能要件7: 無限スクロール（F-04-3）
- 機能仕様7: 無限スクロールで記事を読み込む
  - cursor方式、初期20件、スクロールで追加20件

### 機能要件8: 読み込み中表示
- 機能仕様8: 読み込み中のスケルトン表示: TBD

### 機能要件9: エラー表示
- 機能仕様9: エラー発生時のエラーメッセージ表示: TBD
...
### 非機能要件2: UX
- 非機能仕様3: 読み込み中はスケルトン表示する
- 非機能仕様4: エラー発生時は適切なエラーメッセージを表示する
...
| テスト項目 | 対応仕様 | 観点 | 期待値 |
|------------|----------|------|--------|
| 人気記事取得（メイン） | 機能要件1/機能仕様1, 機能要件2/機能仕様2 | 72時間以内×View数順で記事取得 | 人気記事4件が返される |
| 人気記事取得（右ペイン） | 機能要件1/機能仕様1, 機能要件3/機能仕様3 | 72時間以内×View数順で記事取得 | 人気記事5件が返される |
| ジャンル別記事取得 | 機能要件4/機能仕様4 | ジャンルIDを指定して記事取得 | 該当ジャンルの記事20件が返される |
| ジャンル一覧取得 | 機能要件4/機能仕様4 | ジャンル一覧を取得 | ジャンルリストが返される |
| サイドバー展開状態永続化 | 機能要件6/機能仕様6 | サイドバーの記事タブ展開状態を保存 | user_settingsテーブルに保存される |
| 記事カード表示 | 機能要件5/機能仕様5 | 記事カードに必要な情報が含まれる | タイトル・著者・ジャンル・日時が返される |

### E2Eテスト（実装完了後に記載）

| テストシナリオ | 対応仕様 | 観点 | 期待値 |
|----------------|----------|------|--------|
| 記事ホームアクセスフロー | 機能要件1/機能仕様1, 機能要件2/機能仕様2, 機能要件3/機能仕様3 | ログイン→記事ホーム表示→人気記事表示 | TBD（実装完了後に記載） |
| ジャンル切替フロー | 機能要件4/機能仕様4 | ジャンルタブクリック→ジャンル別記事表示 | TBD（実装完了後に記載） |
| 記事遷移フロー | 機能要件5/機能仕様5 | 記事カードクリック→記事詳細へ遷移 | TBD（実装完了後に記載） |
