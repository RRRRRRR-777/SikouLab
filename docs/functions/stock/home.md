...
### F-03-1: 価格チャート表示
- 価格チャートを表示（ローソク足または折れ線グラフ）
- 期間切り替えボタン: 1日・1週間・1ヶ月・3ヶ月・6ヶ月・1年・YTD・全期間
- チャート上にニュースアイコンをオーバーレイ表示（F-03-2連動）
- 移動平均線の表示: TBD
- ボリューム表示: TBD

### F-03-2: ニュース表示・オーバーレイ
- チャート上にニュースアイコンをオーバーレイ表示
- アイコンクリックでツールチップまたはニュース詳細へ遷移
- 最新ニュース一覧を表示
- ニュースクリックでニュース詳細ページへ遷移
...
### F-03-7: UOA/レーティング表示
- UOA（Unusual Options Activity）一覧
  - オプション種別（call/put）
  - 権利行使価格・満期日
  - 取引量・検出日時
- 証券会社レーティング一覧
  ...
  - 目標株価
  - レーティング日時
- レーティング集計（買い・保有・売りの数）をフロントエンドで計算

### F-03-8: 人気コンテンツ表示
- 直近3ヶ月の人気記事・ニュース
- 関連銘柄に紐付くコンテンツ
- View数順にソート
- 記事・ニュースクリックで詳細ページへ遷移

### F-03-9: アナリストレーティング表示
- アナリスト格付け一覧
  - アナリスト名
  - 格付け（grade）
  - コメント
...
### F-09: ポートフォリオ登録
- ポートフォリオ追加ボタンでポートフォリオ選択モーダルを表示
- 既存ポートフォリオから選択して銘柄を追加
- ポートフォリオ未作成の場合は新規作成を促す
- 登録済みのポートフォリオに含まれる場合は登録済み状態を表示
...
### UX
- 読み込み中はスケルトン表示
- エラー発生時は適切なエラーメッセージ表示
- 銘柄が存在しない場合は404ページへ遷移
...
| テスト項目 | 対応仕様 | 観点 | 期待値 |
|------------|----------|------|--------|
| 銘柄詳細取得 | F-03-1, F-03-3, F-03-4 | 銘柄IDを指定して銘柄詳細を取得 | 銘柄情報・価格・ファンダメンタルズが返される |
| チャートデータ取得 | F-03-1 | 期間を指定して価格履歴を取得 | 指定期間のチャートデータが返される |
| ニュースオーバーレイ表示 | F-03-2 | 銘柄に関連するニュースを取得 | チャート上にニュースアイコンが表示される |
| 人気コンテンツ取得 | F-03-8 | 直近3ヶ月の人気コンテンツを取得 | View数順の記事・ニュースが返される |
| インサイダー取引取得 | F-03-6 | 最近のインサイダー取引を取得 | インサイダー取引一覧が返される |
| アナリストレーティング取得 | F-03-9 | アナリストレーティングを取得 | 価格ターゲット・レーティング集計が返される |
| 期間切り替え | F-03-1 | 異なる期間でチャートデータを取得 | 期間に応じたチャートデータが返される |
| ブックマークトグル | F-08 | ブックマークボタンクリック | ブックマーク状態がトグルされる |
| ポートフォリオ一覧取得 | F-09 | ユーザーのポートフォリオ一覧を取得 | ポートフォリオ一覧が返される |
| ポートフォリオへ銘柄追加 | F-09 | 選択したポートフォリオに銘柄を追加 | 銘柄がポートフォリオに追加される |

### E2Eテスト（実装完了後に記載）

| テストシナリオ | 対応仕様 | 観点 | 期待値 |
|----------------|----------|------|--------|
| 銘柄詳細閲覧フロー | F-03-1〜F-03-9 | 銘柄一覧→銘柄クリック→銘柄詳細表示 | TBD（実装完了後に記載） |
| 期間切り替えフロー | F-03-1 | 銘柄詳細→期間ボタンクリック→チャート更新 | TBD（実装完了後に記載） |
| 人気コンテンツ閲覧フロー | F-03-8 | 銘柄詳細→人気記事クリック→記事詳細へ遷移 | TBD（実装完了後に記載） |
| ブックマーク操作フロー | F-08 | 銘柄詳細→ブックマーククリック→状態更新 | TBD（実装完了後に記載） |
| ポートフォリオ登録フロー | F-09 | 銘柄詳細→ポートフォリオ追加→ポートフォリオ選択→登録完了 | TBD（実装完了後に記載） |
