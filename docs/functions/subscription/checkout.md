...
#### 初回登録フロー（機能要件4/機能仕様1〜3）

| テスト項目 | 対応仕様 | 入力・条件 | 期待値 |
|------------|----------|------------|--------|
| サブスクリプションページが表示される | 機能要件4/機能仕様1 | `/subscription` にアクセス | プラン名・料金・機能一覧が表示される |
| UnivaPayウィジェットが初期化される | 機能要件4/機能仕様2 | 「サブスクリプションを開始する」クリック | UnivaPay checkout が amount・currency・period=monthly で呼ばれる |
| 決済成功後にポーリングが開始される | 機能要件4/機能仕様3 | ウィジェット onSuccess コールバック | `POST /api/v1/univapay/checkout` 呼び出し後、「決済処理中...」が表示される |
| active検知後にダッシュボードへ遷移 | 機能要件4/機能仕様3 | `GET /api/v1/auth/me` が `subscription_status=active` を返す | `/` にリダイレクトされる |
| 409既存登録済みエラー時のトースト表示 | 機能要件4/機能仕様3 | checkout が 409 を返す | 「既に登録済み」のエラートーストが表示される |
| 決済失敗時のエラートースト表示 | 機能要件4/機能仕様3 | ウィジェット onError コールバック | 「決済処理に失敗しました」のエラートーストが表示される |

**注**: タイムアウト処理（30秒経過後のエラー表示・再試行ボタン）は単体テストで検証済みのため、E2Eテストでは割愛。
