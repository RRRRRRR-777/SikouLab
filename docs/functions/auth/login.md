...
| フロントエンド: Firebase SDK導入 | 2 | SDKインストール・初期化・認証関数実装 |
| フロントエンド: 認証画面実装 | 2 | UI実装・OAuthボタン・ローディング状態 |
| フロントエンド: セッション管理 | 2 | Cookie管理・未ログイン時リダイレクト |
| バックエンド: Firebase Admin SDK導入 | 2 | SDKインストール・初期化・ID Token検証 |
| バックエンド: ユーザー登録ロジック | 3 | users/user_settings作成・既存ユーザー判定 |
| バックエンド: Stripe連携 | 2-3 | カスタマー作成・エラーハンドリング |
| バックエンド: ログアウトAPI | 1 | Firebaseサインアウト・セッション削除 |
| バックエンド: セッション確認API | 1 | ユーザー情報返却 |
| 単体テスト | 3 | Firebase/Stripeモック・認証フロー |
| E2Eテスト | 2-3 | 認証フロー・モック必要 |
