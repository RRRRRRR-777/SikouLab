...
### 非機能仕様1: バッチ処理パフォーマンス
- バッチ処理完了: API取得件数によるが5分以内

### 非機能仕様2: 可用性
- 外部API障害時: リトライ、エラーログ記録
...
| テスト項目 | 対応仕様 | 観点 | 期待値 |
|------------|----------|------|--------|
| ニュース取得バッチ | 機能仕様1 | 外部APIからニュースを取得 | ニュースリストが返される |
| 重複チェック | 機能仕様1 | sourceで重複判定 | 既存ニュースはスキップされる |
| 翻訳処理 | 機能仕様2 | タイトル・本文の翻訳 | 日本語訳が返される |
| 翻訳キャッシュ | 機能仕様2 | 同一URLの翻訳結果再利用 | キャッシュされた翻訳が返される |
| ジャンル紐付け | 機能仕様3 | ニュースとジャンルの紐付け | news_genresにレコード作成 |
| 銘柄紐付け | 機能仕様3 | ニュースと銘柄の紐付け | stock_newsにレコード作成 |

### E2Eテスト（実装完了後に記載）

| テストシナリオ | 対応仕様 | 観点 | 期待値 |
|----------------|----------|------|--------|
| バッチ処理フロー | 機能仕様1, 機能仕様2 | 外部API取得→翻訳→DB保存→紐付け | TBD（実装完了後に記載） |
