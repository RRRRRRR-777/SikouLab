...
sequenceDiagram
    ...
    par トレンドニュース
        ...
        API->>DB: 直近24時間以内のpublishedニュース取得
        API->>API: ピン固定ニュースをpin_order順で先頭配置
        API->>API: 残り枠をスコア降順で埋める
        DB-->>API: ニュースリスト
        API-->>Front: トレンドニュース10件（ピン優先）
    and ジャンル別ニュース
...
- 機能仕様1: 直近24時間以内の公開済みニュース（`status=published`）をスコアリング順に最大10個表示する
- 機能仕様2: ピン固定されたニュースを優先表示する
...
**ピン固定の表示ルール**
- `is_pinned=true`のニュースは、スコアに関わらず`pin_order`の昇順（1が最上位）でリストの先頭に配置される
- ピン固定ニュースも24時間以内の制約を受ける（`published_at`が24時間以内）
- ピン固定ニュースが枠を埋めた後、残りの枠をスコア順の通常ニュースで埋める
- 例: ピン2件 + スコア順8件 = 10件表示

**表示順の合成ロジック**
```
1. published_at が 24時間以内 かつ status=published のニュースを取得
2. is_pinned=true のニュースを pin_order 昇順で先頭に配置
3. 残りの枠（10 - ピン件数）を、ピン以外のニュースからスコア降順で埋める
4. 合計10件を返す
```
...
| トレンド取得（ピンあり） | 機能要件1/機能仕様2 | ピン固定ニュース優先表示 | ピンニュースがpin_order順で先頭に配置される |
| トレンド取得（ピン+スコア混合） | 機能要件1/機能仕様2 | ピン2件+スコア順8件 | 合計10件、ピンが先頭 |
| トレンド取得（ピン10件超） | 機能要件1/機能仕様2 | ピン10件以上の場合 | pin_order上位10件のみ表示 |
| トレンド取得（ピン期限切れ） | 機能要件1/機能仕様2 | ピン設定だが24時間超過 | ピンでも24時間超過は非表示 |
...
| F-05-7（ニュース手動投稿） | 手動投稿ニュース（origin=manual）もトレンドの対象 |
| F-12-1-2（ニュース管理） | 管理画面でのピン固定設定がトレンド表示順に影響 |
...
|--------|------------------|------|
| **バックエンド** |||
| トレンドニュースAPI実装 | 4 | スコアリングロジック（View×1 + 銘柄ポートフォリオ登録数×2）、24時間フィルタ、ピン固定優先ロジック、ソート |
...
- ニュース管理（F-12-1-2）でのピン固定設定が反映される前提
- 手動投稿ニュース（F-05-7）もstatus=publishedであればトレンド対象
