...
| サブスク継続率 | 月次解約率10%以下 | UnivaPay Dashboard |
...
| US-13 | アクティブ投資家 | アクティブ投資家として、銘柄の高度な分析情報（UOA、インサイダー取引、アナリストレーティング）を一箇所で確認したい。なぜなら投資判断を迅速に行いたいから。 | F-03 |
| US-14 | アクティブ投資家 | アクティブ投資家として、大手メディアでは報じられないニッチな投資ニュースを読みたい。なぜならこのプラットフォームならではの差別化された情報に価値を感じるから。 | F-05-7 |
...
| F-02-5 | 相場異変レーダー | 全上場銘柄から異常検出した情報を表示 | - | MUST |
| F-03 | 個別銘柄ページ | 株価チャート、ファンダメンタルズ、UOA、レーティング表示 | US-02, US-13 | MUST |
| F-03-1 | 株価チャート | 時系列での株価推移表示 | US-02, US-13 | MUST |
| F-03-2 | ニュースオーバーレイ | チャート上に値動きがあった日のニュースを表示 | US-02, US-13 | MUST |
| F-03-3 | ファンダメンタルズ | EPS、時価総額等の基本指標表示 | US-02, US-13 | MUST |
| F-03-4 | インサイダー取引 | 企業内部者（役員等）の株式売買情報を表示 | US-02, US-13 | MUST |
| F-03-5 | UOA表示 | Unusual Option Activity（大口オプション取引）表示 | US-13 | MUST |
| F-03-6 | 証券会社レーティング | 買い/保有/売り、目標株価の表示 | US-02, US-13 | MUST |
| F-03-7 | 銘柄の関連ニュース | 銘柄に紐づくニュースを日付降順で表示 | US-02, US-07, US-13 | MUST |
| F-03-8 | 人気記事/ニュース | 銘柄に紐づく記事・ニュースを直近3ヶ月×View数で表示 | US-02, US-13 | MUST |
| F-03-9 | アナリスト格付け | Massive APIによる格付け・コメント表示(ポップアップ) | US-13 | MUST |
| F-04 | 記事機能 | 専門家による記事の作成・編集・公開・予約投稿。ジャンル分け、人気記事表示。ニュースとは明確に分離して表示 | US-01, US-08, US-09 | MUST |
| F-04-1 | 記事作成・編集 | Markdownエディタによる記事作成。見出し（大・小・本文）、画像挿入・ドラッグ移動、iframe埋込、目次自動生成、ハイライト機能。投稿ユーザー選択対応 | US-09 | MUST |
...
| F-05 | ニュース機能 | 外部APIからの自動取得に加え、管理者・ライターによる手動投稿にも対応。日本語翻訳、予約投稿、トレンドニュースのピン固定機能。記事とは明確に分離して表示 | US-03, US-11, US-12, US-14 | MUST |
| F-05-1 | ニュース自動取得 | 外部APIからニュースを取得しDBに保存 | US-03 | MUST |
| F-05-2 | 日本語翻訳 | ニュースを日本語に翻訳して表示 | US-03 | MUST |
| F-05-3 | トレンドニュース表示 | 直近24時間以内のニュースをスコアリング順に10個表示。スコア=View数×1pt + 人気銘柄紐付き×2pt。ピン固定されたニュースは優先的に上部に表示し、複数固定時は順序指定可能（10件枠内） | US-11 | MUST |
| F-05-4 | ジャンル別ニュース表示 | 3ジャンル（株式市場・AI・経済等、追加可能）で各5件を最新順に表示。最新1件を大きく表示 | US-12 | MUST |
| F-05-5 | ジャンル詳細ページ | ジャンル別のニュース一覧ページ。フィルター機能（検索・期間・タイプ・センチメント・ティッカー）、外部リンク遷移 | US-12 | MUST |
| F-05-6 | ニュース詳細ページ | ニュース押下時に独自の詳細ページへ遷移し、LLM日本語訳を表示 | - | WANT |
| F-05-7 | ニュース手動作成・編集 | 管理者・ライターによるニュースの新規作成・編集。Markdownエディタで日本語直接入力。投稿ユーザー選択、ジャンル設定、予約投稿対応 | US-14 | MUST |
...
| F-10-2 | サブスクリプション管理 | UnivaPay連携、プラン確認・変更、決済失敗時の権限制限 | - | MUST |
...
| F-12-1-2 | ニュース管理 | 全ニュースの一覧表示、新規作成、編集、削除、予約投稿管理、ピン固定管理。手動投稿ニュースの管理を含む | - | MUST |
...
| F-12-6 | お知らせ通知 | 管理者によるメンテナンス・アップデート・重要ニュースのユーザー通知 | - | MUST |
| F-12-7 | 投稿ユーザー管理 | 記事・ニュースの表示著者となる投稿用ユーザーの作成・編集・削除。管理者のみ操作可能 | - | MUST |
...
|--------|------|
| 管理者 | 全機能・全データへのアクセス |
| 記事投稿権限ユーザー | 記事・ニュースの投稿・自分のコンテンツ編集 |
...
- ユーザーがサイドバーのタブを展開/折りたたみした状態を記録
- 次回ログイン時・ページ遷移時も状態を維持
- ユーザー設定テーブルに`sidebar_article_expanded`（記事タブ）・`sidebar_admin_expanded`（管理タブ）フラグとして保存
...
| 投稿ユーザー | 記事・ニュースの表示上の著者（バイライン）。実際の操作者（ログインユーザー）とは別概念。管理画面で作成・管理する |
| 手動投稿ニュース | 外部APIからの自動取得ではなく、管理者・ライターが直接作成するニュース。大手メディアがカバーしないニッチな情報を提供する目的 |