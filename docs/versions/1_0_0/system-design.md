...
| F-02 | ダッシュボード | ログイン後のホーム画面 | MUST | 設計完了 | `dashboard/home.md` |
| F-02-1 | 相場インデックス表示 | S&P500等の主要指数の価格・変動率を表示 | MUST | 設計完了 | `dashboard/home.md` |
| F-02-2 | 記事・ニュースハイライト | 最新・人気の記事をカード形式で表示 | MUST | 設計完了 | `dashboard/home.md` |
| F-02-3 | ポートフォリオウィジェット | 登録ポートフォリオのパフォーマンスサマリーを表示 | MUST | 設計完了 | `dashboard/home.md` |
| F-02-4 | 関連ニュースウィジェット | ポートフォリオに関連するニュースをリスト表示 | MUST | 設計完了 | `dashboard/home.md` |
| F-02-5 | 相場異変レーダー | 全上場銘柄から異常検出した情報を表示 | MUST | 設計完了 | `dashboard/home.md` |
...
| F-05-7 | ニュース手動作成・編集 | Markdownエディタによるニュース新規作成・編集。投稿ユーザー選択、ジャンル設定、予約投稿対応 | MUST | 設計完了 | `admin/home.md` |
...
| F-10-2 | サブスクリプション管理 | UnivaPay連携、プラン確認・変更 | MUST | 設計完了 | `settings/home.md` |
...
| F-12-7 | 投稿ユーザー管理 | 投稿用ユーザー（表示著者）の作成・編集・削除。管理者のみ | MUST | 設計完了 | `admin/home.md` |
...
| UnivaPay API | 決済・サブスク管理 | F-10-2 | イベント駆動 |
...
UnivaPayのサブスクリプションステータスに基づくアクセス制御。
...
past_dueについて: 請求失敗時にUnivaPayが自動リトライしている期間のステータス。（カード限度額超過、有効期限切れ等が原因）現時点では猶予期間として「全機能利用可能」を想定。
