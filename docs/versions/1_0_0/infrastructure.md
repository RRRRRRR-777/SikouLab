...
SicouLabのインフラはGoogle Cloud上に構築し、おおよそ**月額$54〜91（約8,000〜14,000円）**(運用前なので概算)で本番環境を運用します。

この構成により、記事配信・ニュース自動取得・アンケート・サブスクリプション決済といったサービスの全機能を1つのインフラ上で安定稼働させることができます。800人規模のアクティブユーザーが同時にアクセスしても問題なく動作し、ユーザーが倍増した場合でも追加コストは月額+$10〜20程度に収まります。使った分だけ課金されるサーバーレス構成を採用しているため、アクセスが少ない時間帯のコストはほぼゼロに抑えられ、従来の専用サーバーやVPS（月額2〜5万円）と比較しても同等以上の機能を低コストで実現しています。

データ保護の面では、毎日の自動バックアップに加え約5分単位でのポイントインタイムリカバリに対応しており、万が一の障害時にもデータ損失を最小限に抑えます。サービスの稼働状況は1分間隔で常時監視しており、異常を検知した場合は1分以内に前バージョンへ自動でロールバックできる体制を整えています。SLA目標は99.5%（月間ダウンタイム約3.6時間以内）です。

セキュリティについては、全通信をHTTPS（TLS 1.2以上）で暗号化し、SSL証明書はGoogleによる自動管理で運用負荷をかけません。インフラ層ではCloud Armor（WAF）によるDDoS対策・SQLインジェクション・XSS防御を適用し、認証情報はSecret Managerで厳格に管理することで、アプリケーションコードに秘密情報を一切含めない設計としています。

開発・検証用のdev環境はオンデマンドで起動・停止でき、停止中のコストは$0です。

構築工数は本番環境で40〜56時間、dev環境の追加分で10〜14時間、合計で50〜70時間を見込んでいます。これは、上記のセキュリティ・監視・バックアップ・自動デプロイの仕組みをすべてゼロから構築する工数です。Google Cloudプロジェクトの初期設定からCI/CDパイプラインの構築、Cloud Armorによる多層防御の設定、ダッシュボード・アラートの整備、障害時に1分でロールバックできる運用体制の確立までを含みます。

これらを省略した場合、障害が発生しても検知が遅れ、手動での原因調査・復旧に数時間を要します。セキュリティ対策が不十分であればDDoS攻撃や不正アクセスへの対応が事後になり、ユーザーデータの保護や信頼回復にかかるコストは構築コストを大きく上回ります。また、デプロイの自動化がなければリリースのたびに手作業が発生し、ヒューマンエラーのリスクが継続的に残ります。初期段階で堅牢な基盤を整えることが、長期的なコストと品質の両面で合理的な選択です。

以下、各コンポーネントの詳細設計を記載します。
...
| サービス名 | 役割 | ランタイム | 備考 |
|-----------|------|-----------|------|
| `sicoulab-frontend` | Next.js SSR | Node.js | standalone output mode |
| `sicoulab-api` | Go API | Go | RESTful API |
| `sicoulab-jobs` | バッチ処理 | Go | Cloud Run Jobsで実行 |

### 2.2 リソース設定

#### フロントエンド（sicoulab-frontend）
...
| 形式 | Docker |
| イメージ命名規則 | `asia-northeast1-docker.pkg.dev/<project-id>/sicoulab/<service>:<tag>` |
...
|-----------|------|---------|--------------|
| `sicoulab-assets-prod` | 本番用画像（記事・ユーザーアイコン等） | 公開バケット（直接配信） | なし（永続保持） |
| `sicoulab-vrt-baselines` | VRTベースライン画像 | CI/CDからのみ | 90日で自動削除 |

**ディレクトリ構成**（`sicoulab-assets-prod`）:
```
articles/          - 記事画像
users/             - ユーザーアイコン
news/              - ニュースサムネイル
...
| Backend Service（FE） | Serverless NEG → Cloud Run（sicoulab-frontend） |
| Backend Service（BE） | Serverless NEG → Cloud Run（sicoulab-api） |
| Cloud CDN | 無効（初期） |
...
| `univapay-secret-key` | UnivaPay APIキー | BE |
...
| `doc-check.yml` | PR時 / main push | PR時: docコメントチェック / main push: TypeDoc生成 + Cloudflare Pagesデプロイ |
...
- GitHub OIDC トークンで Google Cloud認証
...
2. Google Cloudコンソール or gcloud CLI で Cloud Run リビジョン一覧を確認
...
**設定場所**: Google Cloudコンソール > Billing > Budgets & Alerts
...
| 環境 | 用途 | ライフサイクル | ドメイン |
|------|------|--------------|---------|
| prod | 本番 | 常時稼働 | `sicoulab.com`（TBD） |
| dev | 開発・検証 | オンデマンド（CD管理） | `dev.sicoulab.com`（TBD） |

> staging環境は初期では構築しない（[ADR-005](../../adr/005-staging-environment.md)）。

### 11.2 リソース名規則

同一Google Cloudプロジェクト内で環境プレフィックスにより分離する。

| リソース | prod | dev |
|---------|------|-----|
| Cloud Run（FE） | `sicoulab-frontend` | `sicoulab-frontend-dev` |
| Cloud Run（BE） | `sicoulab-api` | `sicoulab-api-dev` |
| Cloud Run Jobs | `sicoulab-jobs` | `sicoulab-jobs-dev` |
| Cloud SQL | `sicoulab-db` | `sicoulab-db-dev` |
| Cloud Storage | `sicoulab-assets-prod` | `sicoulab-assets-dev` |
| Cloud Armor | `sicoulab-armor` | `sicoulab-armor-dev` |
...
| ワークフロー | トリガー | 実行内容 |
|------------|---------|---------:|
| `dev-up.yml` | 手動実行 or devブランチpush | Google Cloudリソース作成 → Dockerビルド → デプロイ |
| `dev-down.yml` | 手動実行 | dev環境のGoogle Cloudリソース全削除 |
