...
### 設定機能（F-10）
* docs/functions/settings/email.md
  * 概要: メールアドレス登録（F-10-3）の詳細設計書。ニュースレター受信用メールアドレスの登録・変更機能。
    設定画面および各画面に配置されたウィジェットから登録可能。
    購読状況表示・登録・解除・メールアドレス変更の仕様を定義。
...
* docs/functions/newsletter/home.md
  * 概要: ニュースレター機能（F-11, F-11-1, F-11-2）の詳細設計書。メール登録ユーザーに選択した記事の要約を毎日配信（朝7:30）。
    ...
    シーケンス図・機能要件・非機能要件・テストケースを含む。
* docs/functions/newsletter/manage.md
  * 概要: ニュースレター管理（F-12-3）の詳細設計書。管理者ページで配信リストを管理（最大5記事）し、配信履歴を確認。
    記事投稿時のチェックボックスで配信リストに追加可能。
    画面設計図・シーケンス図・機能要件・テストケースを含む。
