...
* docs/versions/1_0_0/requirements.md
  * 概要: v1.0.0の要件定義書（テンプレート準拠）。「記事購読型コミュニティ」としての核心価値を明記。成功基準（記事閲覧率・継続率含む5指標）、
    ペルソナ（ROM専・投資初心者・アクティブ投資家）、ユーザーストーリー（US-01〜12）、機能一覧（F-01〜F-15、子機能含む計41項目）。
    **更新内容**: ニュースレター機能（F-15）追加、ニュース画面強化（F-02: トレンドニュース10件、ジャンル別表示3カテゴリ各5件、詳細ページ、独自ページ）、
    記事エディタUI詳細化（F-01-1: Markdownエディタ、画像D&D、iframe埋込）、記事とニュースの明確な分離、UI/UX共通要件（ダークモード対応・レスポンシブ対応・カラーコード定義）追加。
