...
## Figma定義
* **配置場所**: Figmaの特定のページ（開発用ページを単一の正とする）
* **作成タイミング**: 詳細設計（/feature-spec）フェーズで作成・更新
* **フロー**:
  1. feature-specでUI仕様を設計
  2. **feature-specを参照して**Figmaに該当画面/コンポーネントを追記
  3. 実装時は**Figmaを正**として実装
* **運用ルール**:
  * 新規画面/コンポーネント追加時は必ずFigmaを先に更新
  * 実装とFigmaに差異が出た場合は**Figmaを正**とする
  * バージョニングはFigmaのバージョン履歴で管理
  * **Figma記載時にわからないことがあれば必ずユーザーに確認する**
  * **Figma記載時に決定した内容は詳細設計書（feature-spec）に反映する**
