...
| テスト項目 | 対応仕様 | 観点 | 期待値 |
|------------|----------|------|--------|
| {項目1} | 機能要件1/機能仕様1 | {観点} | {期待値} |
| {項目2} | 機能要件1/機能仕様1, 機能仕様2 | {観点} | {期待値} |

### E2Eテスト（実装完了後に記載）

| テストシナリオ | 対応仕様 | 観点 | 期待値 |
|----------------|----------|------|--------|
| {シナリオ1} | 機能要件1/機能仕様1, 機能仕様2 | {観点} | {期待値} |
| {シナリオ2} | 機能要件2/機能仕様1 | {観点} | {期待値} |
...
ドキュメント生成後、以下を確認してからユーザーに提示すること。

- [ ] テンプレートに定義されたセクションのみで構成されているか（独自セクションを追加していないか）
- [ ] ER図が `system_datas.md` と一致しているか
- [ ] 機能IDが基本設計書から引用されているか（独自ID未定義）

### IDの扱い

- **`{機能ID}`は基本設計書から引用する**（例: `F-09-1`）。詳細設計で独自IDを新規定義しない
- **`機能仕様N`にはIDを付与しない**。テストケースからは `機能要件N/機能仕様N` 形式で参照する
  - 例: `機能要件1/機能仕様1`、`機能要件1/機能仕様2, 機能仕様3`
- 非機能要件は基本設計書にサブIDがないため、`({機能ID})`は省略可
- **`機能仕様N`の番号は `機能要件N` ごとにリセットして1から始める**（前の機能要件から継続しない）
  - 正: 機能要件1に機能仕様1・2、機能要件2に機能仕様1・2
  - 誤: 機能要件1に機能仕様1・2、機能要件2に機能仕様3・4
