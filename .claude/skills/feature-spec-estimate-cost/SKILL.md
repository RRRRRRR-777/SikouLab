...
機能仕様書に作業見積もりセクションを追加するSkillです。詳細設計の内容から実装コストを見積もり、**4sp = 1時間（実装＋単体テスト＋レビュー、あくまで目安）**としてスプリント計画に活用します。
...
**スプリント計画の目安**: 4sp = 1時間（実装＋単体テスト＋レビューを含む、あくまで参考値）
...
| {タスク3} | {ポイント} | |
| **テスト** |||
| 単体テスト | {ポイント} | API・フロントエンドの単体テスト |
| E2Eテスト | {ポイント} | 主要フローのE2Eテスト |
...
| 絶対値化 | 1sp = 〇日の目安を設定 | 4sp = 1時間（あくまで参考値） |
